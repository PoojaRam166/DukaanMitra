...
const getExpenses = async (req, res, next) => {
  try {
    const result = await db.query('SELECT * FROM expenses WHERE user_id = $1 ORDER BY date DESC, created_at DESC', [req.user.id]);
...
const createExpense = async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      'INSERT INTO expenses (user_id, date, category, description, amount) VALUES ($1,$2,$3,$4,$5) RETURNING *',
      [req.user.id, date || new Date().toISOString().split('T')[0], category, description || '', parseFloat(amount)]
    );
...
const deleteExpense = async (req, res, next) => {
  try {
    const result = await db.query('DELETE FROM expenses WHERE id = $1 AND user_id = $2 RETURNING id', [req.params.id, req.user.id]);
    if (result.rows.length === 0) return res.status(404).json({ success: false, message: 'Expense not found' });
